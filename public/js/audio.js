...
export class Sound {
  ...
  setSfx(on) {
    this.sfxOn = on;
    if (this.ctx) {
      this.sfx.gain.setTargetAtTime(on ? 0.7 : 0, this.ctx.currentTime, 0.1);
      this.rainGain.gain.setTargetAtTime(on ? this.rainLevel * 0.22 : 0, this.ctx.currentTime, 0.5);
    }
  }
  setMood(mood) {
  ...
  setRain(level) {
    ...
    if (this.ctx && this.sfxOn) this.rainGain.gain.setTargetAtTime(level * 0.22, this.ctx.currentTime, 0.8);
  }
  ...
  startRain() {
    ...
    this.rainFilter = lp;
    this.rainGain = ctx.createGain(); this.rainGain.gain.value = this.sfxOn ? this.rainLevel * 0.22 : 0;
  ...
  schedule() {
    ...
    const m = MOODS[this.mood], ahead = ctx.currentTime + 0.5;
    while (this.nextBar < ahead) {
      const chord = m.chords[this.chordIx % m.chords.length];
      this.pad(chord, this.nextBar, m.bar, m.cutoff);
      if (m.brass && this.chordIx % 2 === 0) this.brass(chord.slice(1, 4), this.nextBar + 0.05, m.bar * 0.8);
      this.nextBar += m.bar; this.chordIx++;
    }
    if (this.nextBell < ahead) {
      ...
      this.nextBell = t + 1.2 + Math.random() * 3.5;
    }
